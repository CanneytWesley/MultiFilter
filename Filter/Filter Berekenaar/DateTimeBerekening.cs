...
{
    public class DateTimeBerekening<T> : IBerekening<T>
    {
        ...
        public DateTimeBerekening(string filterTitel, Type filterTrigger, Func<T, DateTime> property, FilterOptie filterOptie)
        {
            Property = property;
        ...
        public List<T> FilterResult(List<T> alleItems, IResult filterresultaat)
        {
            ...
            for (int i = 0; i < logisch.Logica.Count - 1; i += 2)
            {
                ...
                var waarde = new DateTime(Convert.ToInt64(logisch.Logica[i + 1].Waarde));
        ...
        private List<T> Filter(List<T> alleItems, Func<T, DateTime> property, LogischeOperator op, DateTime waarde)
        {
            switch (op)
            {
