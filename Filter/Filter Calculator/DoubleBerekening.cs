...
{
    public class DoubleBerekening<T> : ICalculation<T>
    {
        ...
        public DoubleBerekening(string filterTitel, Type filterTrigger, Func<T, double> property, FilterOption filterOptie)
        {
            Property = property;
        ...
        public List<T> FilterResult(List<T> alleItems, IResult filterresultaat)
        {
            ...
            for (int i = 0; i < logisch.Logic.Count - 1; i += 2)
            {
                ...
                var waarde = Convert.ToDouble( logisch.Logic[i + 1].value);
        ...
        private List<T> Filter(List<T> alleItems, Func<T, double> property, LogicalOperator op, double waarde)
        {
            switch (op)
            {
